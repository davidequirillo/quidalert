...
def test_get_alert_success_general_alert(client, db_session, test_baseuser):
    ...
    # The caller (test_baseuser) is not the alert manager, 
    # because he is not the alert sender (in general alerts, the manager is the sender)
...
def test_get_alert_local_created_by_me(client, db_session, test_baseuser):
    ...
    assert resp_obj["positive_votes_num"] == 0
    assert resp_obj["negative_votes_num"] == 0
    # The caller (test_baseuser) is the alert sender, so he is not an alerted user,
    # and even more, the caller is not the alert manager (in local alerts, the manager is the alerted closest chief)
...
def test_get_alert_not_created_by_me_but_involved(client, db_session, test_baseuser):
    ...
    # The caller (test_baseuser) is an alerted user for this alert, so he is involved in the alert
    # But the caller cannot be the alert manager (because he is not the closest alerted chief)
...
def test_get_alert_called_by_a_chief(client, db_session, test_chief, test_baseuser):
    ...
    assert resp_data["sender_reliability_score"] == baseuser.reliability_score
    assert resp_data["alerted_users_num"] == len(alerted_users)
    # The API caller (test_chief) is not an alerted user in this test, not the alert sender, 
...
def test_get_alert_the_caller_has_voted(client, db_session, test_baseuser):
    ...
    # We select an alert_id of a local alert,
    # where the caller is an alerted user (he is involved in the alert)
    local_alerts = db_session.exec(select(Alert).where(Alert.type == AlertType.local.value)).all()
    local_alerts_ids = [a.id for a in local_alerts]
    assert len(local_alerts) > 0
    statement = select(AlertedUser).where(AlertedUser.user_id == caller.id, AlertedUser.alert_id.in_(local_alerts_ids)) # type: ignore
    alerted_caller = db_session.exec(statement).first()
    # The caller (test_baseuser) is an alerted user for at least one alert (see setup_alerts_data_and_teardown fixture)
    assert alerted_caller is not None
    # We extract the local alert in which the caller is involved (he is an alerted user)
    alert = db_session.exec(select(Alert).where(Alert.id == alerted_caller.alert_id)).first()
    assert alert is not None
    assert alert.type == AlertType.local.value
...
def test_get_alert_the_caller_is_an_alerted_manager(client, db_session, test_chief):
    caller: User = test_chief['user']
    assert caller is not None
    access_token = test_chief['access_token']
    # We select an alert_id of a local-general alert, 
    # where the caller is an alerted user (he is involved in the alert)
    local_alerts = db_session.exec(select(Alert).where(Alert.type == AlertType.local.value)).all()
    local_alerts_ids = [a.id for a in local_alerts]
    assert len(local_alerts) > 0
...
def test_get_alert_the_caller_is_a_sender_manager(client, db_session, test_chief):
    caller: User = test_chief['user']
    assert caller is not None
    access_token = test_chief['access_token']
    # We select an alert_id of a managed alert, 
    # where the caller (test_chief) is the alert sender (he created the alert)
    # (see setup_alerts_data_and_teardown fixture: in that fixture all managed alerts are created by test_chief)
    # It's a non-local alert, so the alert sender is the chief manager
    statement = select(Alert).where(Alert.user_id == caller.id, Alert.type == AlertType.managed.value) # type: ignore
    alert = db_session.exec(statement).first()
    assert alert is not None
    assert alert.type == AlertType.managed.value
    assert alert.user_id == caller.id
    ...
    assert resp_data["alert"]["type"] == alert.type
    assert resp_data["alert"]["description"] == alert.description
    # The API caller (test_chief) is the alert sender in this test, and he is also the alert manager,
    # (because in non-local alerts, the alert sender is the chief manager)
    # Obviously, he is not an alerted user, because he is the alert sender
    assert resp_data["user_is_alerted"] is False
    assert resp_data["user_is_sender"] is True
    assert resp_data["user_is_manager"] is True
    assert resp_data["user_vote"] == 0
