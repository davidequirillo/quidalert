...
- Remember to run the GPS location test to ensure your device's location tracking system is working properly, allowing you to send and receive alerts for nearby locations. There's also a log of the latest GPS locations detected locally in the background, allowing you to verify proper operation. Interestingly, only some of these locations will be sent to the server (only those that meet certain requirements: sufficient distance traveled, sufficient time elapsed based on the type of activity detected, stationary mode detected, etc.)
