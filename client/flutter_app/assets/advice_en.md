...
- Remember to run the GPS location test to ensure your device's location tracking system is working properly, allowing you to send and receive alerts for nearby locations. There's also a log of the latest GPS locations detected locally in the background, allowing you to verify proper operation. Interestingly, only some of these locations will be sent to the server (only a part of those detected as "not moving", and some locations in movement at low speed).
